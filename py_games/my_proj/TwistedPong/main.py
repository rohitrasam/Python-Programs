...
class Game:
    ...
    def __init__(self):
        ...
        self.ball = Ball(
            self,
            (self.get_display_width() // 2, self.get_display_height() // 2),
            size=16,
        )
        self.assets = {
        ...
        self.entities: list[Player, Bot | Player] = [
            self.player,
            Bot(
                self,
                [self.get_display_width() - 7, self.get_display_height() // 2],
                [5, 32],
                3,
            ),
        ]
        ...
        self.select = Button(
            self,
            "ball",
            [self.get_display_width() // 2 - 25, self.get_display_height() // 2],
            size=[16, 16],
        )
        self.title = Button(
            self, "title", [self.get_display_width() // 2, 50], size=[116, 29]
        )
        ...
        self.buttons = [
            Button(
                self,
                "2P",
                [self.get_display_width() // 2, self.get_display_height() // 2 + 30],
            ),
            Button(
                self,
                "cpu",
                [self.get_display_width() // 2, self.get_display_height() // 2 + 55],
            ),
            Button(
                self,
                "exit",
                [self.get_display_width() // 2, self.get_display_height() // 2 + 80],
            ),
        ]
        self.counters = [
