    ...
    def __init__(self) -> None:
        ...
        self.assets = {
            "bg": pg.transform.scale(
                self.load_image("space_background_pack/parallax-space-1.png"),
    ...
    def main_game(self):
        ...
        for row in range(self.stars_rows):
            for col in range(self.stars_cols):
                self.display.blit(
                    self.assets["stars"],
                    (row * self.stars_width + self.scroll, col * self.stars_height),
                )
        ...
        self.display.blit(
            self.assets["rings"], (WIDTH / 2 + WIDTH / 4, HEIGHT / 2 + HEIGHT / 4)
        )
