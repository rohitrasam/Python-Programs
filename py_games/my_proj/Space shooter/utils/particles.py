...
class Smoke:
    ...
    def light_effect(self, color, radius):
    ...
    def render(self):
        ...
        light_radius = self.radius*2
        self.game.display.blit(self.light_effect((20,)*3, light_radius), (self.pos.x-light_radius, self.pos.y-light_radius), special_flags=pg.BLEND_RGB_ADD)
        self.update()
...
class Spark:
    ...
    def render(self):
        points = [
                [self.pos[0] + math.cos(self.angle)*self.scale*self.speed, self.pos[1] + math.sin(self.angle)*self.scale*self.speed],
